...
// The animation uses a CurvedAnimation to apply easing curves for smoother motion.
    ...
    final randFloat = useState<double>(0.00);

    useEffect(() {
      final subscription = selected.listen((event) {
        selectedIndex.value = event;
        randFloat.value = ((_math.Random.secure().nextBool() ? 0.01 : -0.01) * _math.Random.secure().nextInt(49) );
        animate();
      });
            ...
            AnimatedBuilder(
              ...
              builder: (context, _) {
                ...
                return LayoutBuilder(builder: (context, constraints) {
                  ...
                  final selectedAngle =
                      -2 * _math.pi * ((selectedIndex.value - randFloat.value) / items.length);
                  final panAngle =
